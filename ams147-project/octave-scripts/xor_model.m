...
function retval = xor_model(network_topology, max_iters, tolerance, validation_cases)
  zinhart; % load module
...
%{
  ...
  %for current_iter = 1 : max_iters  %training loop begin
    ...
    for ith_target = 1: output_layer_size
      targets(ith_target .- 1) = logical_xor(input_t); %get target for specific input
    endfor
  ...
  title("Figure 1: Learning The XOR Gate Training");
  xlabel("Iteration");
  ...
  title("Figure 2: Learning The XOR Gate Decision Boundary");
  xlabel("Model Output");
  ylabel("Target Value");
  hold off;
  clear zinhart; 
